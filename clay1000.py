...
class Clone:
    ...
    async def categories_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for channel in channels:
            try:
                ...
                print_add(f"Katagori Açıldı: {channel.name}")
            except discord.Forbidden:
            ...
            try:
                for category in guild_to.categories:
                    try:
                        ...
                        print_warning(f"Kanal {channel_text.name} Herhangi Bir Kategorisi Yok!")
                        category = None
                        break
    ...
    async def emojis_delete(guild_to: discord.Guild):
        for emoji in guild_to.emojis:
            try:
                ...
                print_error(f"Emoji Silinirken Hata Oluştu: {emoji.name}")
            except discord.HTTPException:
                print_error(f"Emoji Silinirken Hata Oluştu: {emoji.name}")
    ...
    async def emojis_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for emoji in guild_from.emojis:
            try:
                ...
                print_error(f"Emoji Oluştururken Hata: {emoji.name} ")
            except discord.HTTPException:
                print_error(f"Emoji Oluştururken Hata: {emoji.name}")
    ...
    async def guild_edit(guild_to: discord.Guild, guild_from: discord.Guild):
        try:
            ...
            print_error(f"Sunucu Simgesini Değiştirirken Hata Oluştu: {guild_to.name}")